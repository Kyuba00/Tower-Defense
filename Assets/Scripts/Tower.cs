...
public class Tower : MonoBehaviour
{
    ...
    private float _runningShotDelay;
    private Enemy _targetEnemy;
    private Quaternion _targetRotation;

    // Digunakan untuk menyimpan posisi yang akan ditempati selama tower di drag
    public Vector2? PlacePositiion { get; private set; }

    public void SetPlacePosition(Vector2? newPosition)
    {
        PlacePositiion = newPosition;
    }

    public void LockPlacement()
    {
        transform.position = (Vector2)PlacePositiion;
    }
    ...
    public void ShootTarget()
    {
        ...
        _runningShotDelay -= Time.unscaledDeltaTime;
        if (_runningShotDelay <= 0f)
        {
